<?php
class SalesItem extends DatabaseObject {

	protected static $tablename = 'salesitems';
	protected static $classname = 'SalesItem';
	...
	public static function findByIdTranslated($id, $languagecode) {

		if (!is_numeric($id)) die();
		$sql = "SELECT a.`id`, CASE WHEN ISNULL(b.`languagecode`) THEN a.`name` ELSE b.`name` END AS `name`, a.`price`
			FROM `". DB_PREFIX. self::$tablename. "` AS a
			LEFT OUTER JOIN `". DB_PREFIX. "salesitemlangs` AS b ON a.`id` = b.`salesitemid` AND b.`languagecode` = ?
			WHERE a.`id` = ? LIMIT 1;";
		$params[] = $languagecode;
		$params[] = $id;
		$resultArray = self::findBySQL($sql, $params);
