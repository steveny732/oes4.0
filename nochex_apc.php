<?php
/* ************************************************************************
' nochex_apc.php - Nochex application callback (adapted from Nochex example)
'
'	Version: 1.0
'	Last Modified By:
...
$p_ordernumber = $_POST['order_id'];

if ($currentSite->paylogactive) {
	$paylog = new PayLog();
	$paylog->provider = 'NOCHEX';
	$paylog->ordernumber = $p_ordernumber;
	$paylog->posts = 'APC session started';
	$paylog->save();
}
...
foreach ($_POST as $key => $value) {
	$urlencoded .= $prefix. urlencode($key). '='. urlencode($value);
	if (empty($prefix)) {
		$prefix = '&';
	}
}
...
// stores the response from the Nochex server for logging
