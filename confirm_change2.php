<?php
/* ************************************************************************
' confirm_change2.php -- Confirm change #2 - Perform update
'
...
$transfer = Transfer::findByIdForUpdate($_REQUEST['changeid']);
...
foreach ($columns as $column) {
	...
	if ($dbName != 'id' &&
		...
		/* One final check of the dbcolumn persistence flag and we're good to go. If the column is
		   on the transfer table, always update. If it isn't, and the changepersist flag is NOT set
		   leave the entries column untouched. We're checking for changepersist being not set as
		...
		if (isset($transfer->$transferColName) ||
			(!isset($transfer->$transferColName) && !$dbcolumn->changepersist)) {
			...
			if (((!empty($entry->$dbName) && $entry->$dbName <> '0000-00-00 00:00:00') || !empty($p_data)) &&
				...
				$change->source = 'Change '. $transfer->code;
...
redirect_to('confirm_change3.php');
?>